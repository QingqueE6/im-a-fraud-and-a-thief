namespace Bowling4
{
    class Program
    {
        ...
        static bool[][] Pins = new bool[TotalRounds][];
        ...
        static void InitializePins()
        {
            ...
            {
                Pins[i] = Enumerable.Repeat(true, 10).ToArray();
            }
        }

        static void PlayLanes()
        {
            for (int i = 0; i < TotalRounds - 1; i++)
            {
                Console.Clear();
                Console.WriteLine($"Round {i + 1}: Choose a lane, {Username}!");
                ShowPinsNew(i);
        ...
        static void Throw(int i)
        {
            ...
            switch (keyInfo.KeyChar)
            {
                ...
                case '2':
                    ...
                    break;
                case '3':
                    if (Pins[i][1] == false) Pins[i][7] = false;
                    else Pins[i][1] = false;
                    break;
                case '4':
                    if (Pins[i][0] == false) Pins[i][4] = false;
                    else Pins[i][0] = false;
                    break;
                case '5':
                    if (Pins[i][2] == false) Pins[i][8] = false;
                    else Pins[i][0] = false;
                    break;
                case '6':
                    Pins[i][5] = false;
                ...
                case '7':
                    ...
                    break;
                default:
                    Console.WriteLine("You entered an invalid input! Do it again.");
                    Throw(i);
                    break;
            }
            Console.Clear();
        ...
        static void PlayFinal()
        {
            
        }

        static void ShowPinsNew(int i)
        {
            string PinStr(int index) => Pins[i][index] ? "o" : " ";
            Console.WriteLine($"{PinStr(6)}   {PinStr(7)}   {PinStr(8)}   {PinStr(9)}");
            Console.WriteLine($"  {PinStr(3)}   {PinStr(4)}   {PinStr(5)}  ");
            Console.WriteLine($"    {PinStr(1)}   {PinStr(2)}    ");
            Console.WriteLine($"      {PinStr(0)}      ");
        }
